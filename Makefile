...
images: image-node18-npm-toolset ## Build images.
.PHONY: images

image-node18-npm-toolset: ## Build node18-npm-toolset image.
	docker build \
        -f build/images/Dockerfile.node18-npm-toolset \
		-t localhost:5000/ods-pipeline/node18-npm-toolset \
		.
.PHONY: image-node18-npm-toolset
